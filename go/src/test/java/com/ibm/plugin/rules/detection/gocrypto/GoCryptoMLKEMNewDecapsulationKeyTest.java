...
class GoCryptoMLKEMNewDecapsulationKeyTest extends TestBase {
    ...
    void test() {
        GoVerifier.verify(
                "rules/detection/gocrypto/GoCryptoMLKEMNewDecapsulationKeyTestFile.go", this);
    }
    ...
    public void asserts(
        ...
        assertThat(value0.asString()).isEqualTo("ML-KEM-768");

        // KeySize from seed parameter (64 bytes = 512 bits)
        ...
        assertThat(value01.asString()).isEqualTo("512");
        ...
        assertThat(kemNode.getChildren()).hasSize(3);
        assertThat(kemNode.asString()).isEqualTo("ML-KEM-768");
        ...
        assertThat(paramSetNode.asString()).isEqualTo("768");

        // Oid under KeyEncapsulationMechanism (added by enricher)
        INode oidNode = kemNode.getChildren().get(Oid.class);
        assertThat(oidNode).isNotNull();
        assertThat(oidNode.asString()).isEqualTo("2.16.840.1.101.3.4.4.2");

        // KeyLength under KeyEncapsulationMechanism (from seed size: 64 bytes = 512 bits)
        INode keyLengthNode = kemNode.getChildren().get(KeyLength.class);
